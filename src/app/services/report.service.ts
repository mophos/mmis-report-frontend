...
export class ReportService {
  ...
  async getInfo(reportId) {
    const rs: any = await this.authHttp.get(`${this.url}/reports/info?reportId=${reportId}`)
      .toPromise();
    return rs.json();
  }

  async remove(reportId) {
    const rs: any = await this.authHttp.delete(`${this.url}/reports?reportId=${reportId}`)
      .toPromise();
    return rs.json();
  }
  ...
  async saveHeader(header) {
    const rs: any = await this.authHttp.post(`${this.url}/reports/header`, {
      header: header
    })
      .toPromise();
    return rs.json();
  ...
  async saveDetail(detail) {
    const rs: any = await this.authHttp.post(`${this.url}/reports/detail`, {
      detail: detail
    })
      .toPromise();
    return rs.json();
